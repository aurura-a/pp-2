...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.weight = random.choice([1, 2, 3])  # Random weight values
        self.original_image = pygame.image.load(
            f"racer_folder/Coin{self.weight}.png")
...
# Function to increase speed when collecting coins


def check_level():
    global SPEED, LEVEL
    if coins_collected // 4 > LEVEL:
        LEVEL += 1
        SPEED += 2
...
while True:
    ...
    DISPLAYSURF.blit(level_display, (SCREEN_WIDTH -
                     level_display.get_width() - 10, 40))
    ...
    if coin_collision:
        ...
        for coin in coin_collision:
            coins_collected += coin.weight  # Increase coins by weight
        new_coin = Coin()
